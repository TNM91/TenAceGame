...
try{
 ...
 for(const next of ['run','stand','forehand','backhand','serve'])document.getElementById(next).onclick=()=>{mode=next;time=0;event=null;phase.value=0;phase.disabled=!['forehand','backhand','serve'].includes(mode);for(const id of ['run','stand','forehand','backhand','serve'])document.getElementById(id).setAttribute('aria-pressed',String(id===mode));paused=false;pause.textContent='Pause motion';pause.setAttribute('aria-pressed','false');};

...
let angle=.22,pitch=0,close=false,paused=false,drag=null,last=0,renderer,rig,mode='run',time=0,event=null;
function cameraPose(){const radius=close?1.6:4.2,focus=close?1.8:1.08;camera.position.set(Math.sin(angle)*radius,focus+.12+pitch,Math.cos(angle)*radius);camera.lookAt(0,focus,0);}
...
try{
 ...
 for(const next of ['run','stand','forehand','backhand','serve'])document.getElementById(next).onclick=()=>{mode=next;time=0;event=null;for(const id of ['run','stand','forehand','backhand','serve'])document.getElementById(id).setAttribute('aria-pressed',String(id===mode));paused=false;pause.textContent='Pause motion';pause.setAttribute('aria-pressed','false');};

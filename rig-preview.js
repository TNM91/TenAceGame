...
let renderer,rig,mode='ready',close=false,angle=.15,pitch=0,event=null,eventAt=0,last=performance.now(),time=0,drag=null;
...
function startPose(next){mode=next;eventAt=time;event=next==='forehand'||next==='backhand'||next==='serve'?{point:new T.Vector3(next==='backhand'?.42:-.42,next==='serve'?2.18:1.38,.30),time,shot:next==='serve'?'serve':'topspin'}:null;document.querySelectorAll('[data-pose]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.pose===next)));}
for(const b of document.querySelectorAll('[data-pose]'))b.onclick=()=>startPose(b.dataset.pose);
...
try{
 ...
 function frame(now){const dt=document.hidden?0:Math.min(.05,(now-last)/1000);last=now;time+=dt;if(!document.hidden){if(event&&time-eventAt>2.2)startPose(mode);rig.pose({x:.5,y:.5,tx:.5,near:false,ready:true,preview:mode,shot:'topspin',serve:mode==='serve'&&time-eventAt>1.0,swing:event?Math.max(0,1-(time-eventAt)*3):0},time,dt,event);renderer.render(scene,camera);}requestAnimationFrame(frame);}requestAnimationFrame(frame);

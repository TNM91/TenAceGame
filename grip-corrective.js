...
export function closeRacketHand(model){
 model.updateMatrixWorld(true);const hand=model.getObjectByName('RightHand'),inverse=hand.matrixWorld.clone().invert();let changed=0;
 model.traverse(mesh=>{
  ...
  for(let i=0;i<p.count;i++){
   ...
   if(local.distanceToSquared(original)>1e-10){local.applyMatrix4(fromHand);p.setXYZ(i,local.x,local.y,local.z);changed++;edited.add(i);}
  }
